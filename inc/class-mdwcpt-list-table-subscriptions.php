...
if ( ! class_exists( 'MDWCPT_List_Table_Subscriptions' ) ) {

	require_once( MDWCPT_DIR . '/inc/abstracts/abstract-class-mdwcpt-list-table.php' );

	/**
	 * Displaying a list of subscriptions in an ajaxified HTML table.
	 */	
	class MDWCPT_List_Table_Subscriptions extends MDWCPT_List_Table {
		...
		public function __construct() {
			parent::__construct( array (
				'singular' => 'mdwcpt_subscription',
				'plural' => 'mdwcpt_subscriptions',
			) );
			...
			if ( isset( $_GET['status'] ) && 'queued' === $_GET['status'] && ! $this->counts['queued'] && $this->counts['sent'] && 2 === count( $_GET ) ) {
				wp_redirect( self_admin_url( 'admin.php?page=mdwcpt-subscriptions&status=sent' ) );
				exit;
			}
		}
		...
		public function prepare_items() {
			...
			if ( isset( $_GET['s'] ) && is_string( $_GET['s'] ) && ! empty( $_GET['s'] ) ) {
				...
				} elseif ( MDWCPT_Utils::is_absint( $_GET['s'] ) ) {
		...
		public function get_columns(){
			$columns = array(		
				...
				'product' 	    => __( 'Product', 'mdwcpt' ),
				'current_price' => __( 'Current price', 'mdwcpt' ),			
				'expected_price'=> __( 'Expected price', 'mdwcpt' ),
				'email'      	=> __( 'User', 'mdwcpt' ),
				'created'	    => __( 'Created', 'mdwcpt' ),
				'updated'	    => __( 'Updated', 'mdwcpt' ),
			);
			if ( isset( $_GET['status'] ) && 'sent' === $_GET['status'] ) {
				$columns['updated'] = __( 'Sent', 'mdwcpt' );
			} elseif ( isset( $_GET['status'] ) && 'queued' === $_GET['status'] ) {
				$columns['updated'] = __( 'Queued', 'mdwcpt' );
			}
			return $columns;
		}
		...
		public function column_product( $item ) {		
			...
			$actions = array(
				'edit'   => sprintf('<a target="_blank" href="%s">%s</a>', esc_url( $edit_url ), __('Edit product', 'mdwcpt')),
				'delete' => sprintf(
					...
					__('Delete subscription', 'mdwcpt')
				),
			);		
						
			$variation = $this->_products[ $item['id'] ]->is_type('variation') ? sprintf( '<small> (%s)</small>', __( 'variation', 'mdwcpt' ) ) : '';
				
			return sprintf(
				'<a title="%s" target="_blank" href="%s">%s</a>%s%s',
				__('View product', 'mdwcpt'),
		...
		public function column_current_price( $item ) {
			...
			$price .= $this->_products[ $item['id'] ]->is_on_sale() 
				? sprintf(' <small class="mdwcpt-sale">[%s]</small>', __( 'now on sale', 'mdwcpt' ) ) : '';
		...
		public function get_views() {		
			...
			$links = array(
				'active' => __( 'Active', 'mdwcpt' ),
				'queued' => __( 'Queued to be sent', 'mdwcpt' ),
				'sent'   => __( 'Sent', 'mdwcpt' )			
			);
			...
			foreach( $links as $status => $label ) {
				...
				if ( ( isset( $_GET['status'] ) && $_GET['status'] === $status ) || ( 'active' === $status && ! isset( $_GET['status'] ) ) ) {
					...
					$url = ( 'active' === $status )
						? self_admin_url( 'admin.php?page=mdwcpt-subscriptions' ) 
						: self_admin_url( 'admin.php?page=mdwcpt-subscriptions&status=' . $status );
				}
		...
		public function get_bulk_actions() {		
			...
			$actions = array( 'delete' => __( 'Delete subscriptions', 'mdwcpt' ));
			
			if ( ! isset( $_GET['status'] ) )
				$actions['reduce'] = __( 'Reduce prices to expected', 'mdwcpt' );
		...
		protected function process_bulk_action() {
			...
			if ( 'delete' === $action && ! empty( $ids ) ) {           			
				...
				foreach( $filtered as $item ) {
					...
					if ( $product->is_on_sale() )
						$product->set_sale_price( (float) $item['price'] );
					else 
			...
			if ( isset( $count ) && $count > 0 ) {
				update_option( 'mdwcpt_subscriptions_processed_' . $action, $count, false );
				$redirect_url = self_admin_url( 'admin.php?page=mdwcpt-subscriptions&mdwcpt-action=' . $action );
